...
public class creatorBall : MonoBehaviour
{
    ...
    /// 从本地加载对应关卡数据
    /// </summary>
    /// <param name="currentLevel">当前要加载的关卡</param>
    /// <returns></returns>
    ...
    /// 解析关卡数据
    /// </summary>
    /// <param name="mapText">关卡文本数据</param>
    void ProcessGameDataFromString(string mapText)
    {
        ...
        {
            if (line.StartsWith("MODE "))// 获取关卡模式
            {
                string modeString = line.Replace("MODE", string.Empty).Trim();
                LevelData.mode = (ModeGame)int.Parse(modeString);
            }
            else if (line.StartsWith("SIZE "))// 获取关卡的行数和列数
            {
            ...
            else if (line.StartsWith("STARS "))// 每颗星星获取对应的分数
            {
    ...
    /// 获取不同的ball对象设置的tag
    /// </summary>
    /// <returns>要给Game Object 设置的tag</returns>
